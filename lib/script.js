(async () => {
    ...
    function clientSideLimits(input, maxLength = 1000) {
        return input
            .slice(0, maxLength)
            .replace(/[\x00-\x1F\x7F]/g, '') // Remove problematic chars
    }
    ...
    async function processKeyTerms(keyTerms) {
        ...
        console.log("Extracted Key-Term Pairs:", keyTermPairs);
        ...
        console.info("Dialogs created: %o", dialogs);
        contentObj.dialogs = dialogs;
        console.info("Content object updated: %o", contentObj);
